		...
		ctx.fillRect(0, 0, 750, 1150);
 		uni.getImageInfo({
 			...
 			success: function(res) {
				...
				if(arr2[0]  == '/static/images/posterbackgd.png'){
					const WIDTH = res.width;
					const HEIGHT = res.height;
					...
					ctx.setFontSize(44);
					ctx.setFillStyle('#000');
					ctx.fillText( arr2[3], 156, 176 );
					
					console.log('111111111aaaaa')
					//绘制的头像宽度
					let avatarurl_width = 100
					//绘制的头像高度
					let avatarurl_heigth = 100
					//绘制的头像在画布上的位置
					let avatarurl_x = 40
					//绘制的头像在画布上的位置
					let avatarurl_y = 136
					...
					ctx.clip()
					// 绘制图片
					ctx.drawImage(arr2[4], avatarurl_x, avatarurl_y, avatarurl_width, avatarurl_heigth)
					// 恢复之前保存的绘图上下文
					ctx.restore()
					
					ctx.drawImage(arr2[1], WIDTH * 0.05, 260, WIDTH * 0.9, WIDTH * 0.9);
					ctx.save();
					...
					for (let m = 0; m < contentArray.length; m++) {
						if (m) {
							ctx.fillText(contentArray[m], 40, 1055 + contentHh * m + 0,1180);
						} else {
							ctx.fillText(contentArray[m], 40, 1055 + contentHh * m,1180);
						}
					}
					
					let CONTENT_ROW_LENGTH1 = 28;
					let [contentLeng1, contentArray1, contentRows1] = that.textByteLength(arr2[5], CONTENT_ROW_LENGTH1);
					if (contentRows1 > 2) {
						contentRows1 = 2;
						let textArray = contentArray1.slice(0, 2);
						textArray[textArray.length - 1] += '……';
						contentArray1 = textArray;
					}
					ctx.setTextAlign('left');
					ctx.setFontSize(24);
					ctx.setFillStyle('#999999');
					let contentHh1 = 36;
					for (let m = 0; m < contentArray1.length; m++) {
						if (m) {
							ctx.fillText(contentArray1[m], 40, 1085 + contentHh1 * m + 0,1180);
						} else {
							ctx.fillText(contentArray1[m], 40, 1085 + contentHh1 * m,1180);
						}
					}
 	...
 	uploadImageOne: function(opt, successCallback, errorCallback) {
 		...
 		uni.chooseImage({
 			...
 			success: function(res) {
				...
				requestDef.get('oss/sign',{
					filePath:res.tempFilePaths[0],
				},{noVerify: true}).then(defRes => {
					uni.uploadFile({
						url: defRes.requestURL,
						filePath: res.tempFilePaths[0],
						fileType: fileType,
						name: 'file',
						formData: {
						   "key": defRes.filename,
						   "policy": defRes.policyBase64,
						   "OSSAccessKeyId": defRes.accessId,
						   "signature": defRes.signature,
