...
<script>
	...
	import {UserGoodsPayPrice} from '@/api/user'
	var _self;
	export default {
		name: 'user-healthy',
		props: {
		...
		data() {
			 const currentDate = this.getDate({  format: 'yyyy-mm'  })
			return {
	...
	console.log(111)
	this.UserGoodsPayPrice(this.date);
		},
		onShow() {
		...
		methods: {
			...
			UserGoodsPayPrice(date){
				UserGoodsPayPrice({"date":date}).then(res => {
